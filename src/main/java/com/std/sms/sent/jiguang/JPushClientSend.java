...
public class JPushClientSend {
    ...
    //分组推送
    public static boolean toSendTagPush(String appKey, String masterSecret,
            String tag, String noticeInfo) {
    	boolean res = false;
    	ClientConfig clientConfig = ClientConfig.getInstance();
        JPushClient jpushClient = new JPushClient(masterSecret, appKey, null,
            clientConfig);
        ...
        if (StringUtils.isNotBlank(tag)) {
            payload = buildPushObject_android_and_ios_tag_alert(noticeInfo,
                apnsProduction, tag);
        } 
        try {
            PushResult result = jpushClient.sendPush(payload);
    ...
    public static PushPayload buildPushObject_android_and_ios_tag_alert(
    		String ALERT, boolean APNS_PRODUCTION, String... tag1) {
    	return PushPayload
                .newBuilder()
                .setPlatform(Platform.all())
                .setAudience(
